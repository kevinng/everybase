...
class ContactInlineAdmin(admin.TabularInline):
    ...
    def contact_link(self, obj):
        ...
        link += f'/leads/contact/{obj.id}/change'
        return format_html(f'<a href="{link}" target="{link}">{obj.first_name} {obj.last_name}, {obj.country.name}</a>')

_contact_note_fields = ['relevance', 'contact', 'body']
@admin.register(models.ContactNote)
class ContactNoteAdmin(comadm.StandardAdmin):
    list_display = comadm.standard_list_display + _contact_note_fields
    list_editable = [] # To speed up loading
    search_fields = comadm.standard_search_fields + ['body']

    # Details page settings
    fieldsets = comadm.standard_fieldsets + \
        [('Details', {'fields': _contact_note_fields})]
    autocomplete_fields = ['contact']
...
class LeadAdmin(comadm.StandardAdmin):
    ...
    # Details page settings
    readonly_fields = comadm.standard_readonly_fields + ['uuid', 'slug_link', 'slug_tokens', 'num_spam_flags', 'num_scam_flags', 'num_contacts']
    fieldsets = [
        (None, {'fields': ['id', 'uuid']}),
        ('Details', {'fields': ['author', 'lead_type', 'body']}),
        ('Meta', {'fields': ['has_insights', 'internal_notes', 'slug_link', 'slug_tokens', 'impressions', 'num_spam_flags', 'num_scam_flags', 'num_contacts', 'clicks']}),
        ('Timestamps', {'fields': ['created', 'updated', 'deleted']}),
        ('Not in use', {'fields': [
            'headline', 'details', 'commission_type', 'commission_usd_mt', 'min_commission_percentage', 'max_commission_percentage', 'buy_country', 'sell_country', 'author_type', 'questions',
            'title', 'currency', 'comm_details', 'need_agent', 'country', 'agent_job', 'avg_deal_size', 'commission_payable_by', 'commission_payable_after', 'commission_payable_after_other',
            'commission_earnings', 'commission_quantity_unit_string', 'commission_type_other', 'other_comm_details', 'is_comm_negotiable', 'is_promoted', 'question_1', 'question_2', 'question_3',
    ...
    def num_spam_flags(self, obj):
        return models.LeadFlag.objects.filter(
            deleted__isnull=True,
            lead=obj,
            type='spam'
        ).count()

    def num_scam_flags(self, obj):
        return models.LeadFlag.objects.filter(
            deleted__isnull=True,
            lead=obj,
            type='scam'
